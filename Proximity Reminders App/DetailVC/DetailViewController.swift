...
//    @IBOutlet weak var detailDescriptionLabel: UILabel!
    ...
    func saveReminder() {
        ...
        guard let title = self.titleTextField.text, !title.isEmpty else { self.generalAlert(title: "Missing Title", message: "Title for reminder is required."); return }
        guard let location = self.locationPoint else { self.generalAlert(title: "Missing Location", message: "Location for reminder is required."); return }
        ...
        if self.locationManager.monitoredRegions.count == 20 {
            self.generalAlert(title: "Max Reminders Reached", message: "Apps cannot monitor more than 20 regions at any given time. Please delete atleast one reminder before you add another.")
            return
        }
